...
#pragma pack(pop)
    ...
    private:
        static ComPtr<ID3D12RootSignature> CreateGenMipsRootSignature(_In_ ID3D12Device* device)
        {
        ...
        static ComPtr<ID3D12PipelineState> CreateGenMipsPipelineState(_In_ ID3D12Device* device,
...
public:
    ...
    void Begin(D3D12_COMMAND_LIST_TYPE commandType)
    {
        ...
        ThrowIfFailed(
            mDevice->CreateCommandList(1, commandType, mCmdAlloc.Get(), nullptr, IID_GRAPHICS_PPV_ARGS(mList.ReleaseAndGetAddressOf())));
    ...
    void Transition(_In_ ID3D12Resource* resource, _In_ D3D12_RESOURCE_STATES stateBefore, _In_ D3D12_RESOURCE_STATES stateAfter)
    {
        if (!mInBeginEndBlock)
            throw std::logic_error("Can't call Upload on a closed ResourceUploadBatch.");
        ...
        if (mCommandType == D3D12_COMMAND_LIST_TYPE_COPY)
        {
            ...
            case D3D12_RESOURCE_STATE_COPY_SOURCE: break;
...
private:
    ...
    void GenerateMips_UnorderedAccessPath(_In_ ID3D12Resource* resource)
    {
        ...
        for (uint32_t mip = 1; mip < desc.MipLevels; ++mip)
        {
            ...
            mList->SetComputeRoot32BitConstants(GenerateMipsResources::Constants, GenerateMipsResources::Num32BitConstants, &constants, 0);

            // Process this mip
            mList->Dispatch((mipWidth + GenerateMipsResources::ThreadGroupSize - 1) / GenerateMipsResources::ThreadGroupSize,
    ...
    void GenerateMips_TexturePath(_In_ ID3D12Resource* resource)
    {
        ...
        const D3D12_RESOURCE_STATES originalState
            = mCommandType == D3D12_COMMAND_LIST_TYPE_COMPUTE ? D3D12_RESOURCE_STATE_COPY_DEST : D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
    ...
    void GenerateMips_TexturePathBGR(_In_ ID3D12Resource* resource)
    {
        ...
        aliasDesc.Format = (resourceDesc.Format == DXGI_FORMAT_B8G8R8X8_UNORM || resourceDesc.Format == DXGI_FORMAT_B8G8R8X8_UNORM_SRGB) ?
...
_Use_decl_annotations_ void ResourceUploadBatch::Upload(ID3D12Resource* resource, const SharedGraphicsResource& buffer)
{
...
ResourceUploadBatch::Transition(ID3D12Resource* resource, D3D12_RESOURCE_STATES stateBefore, D3D12_RESOURCE_STATES stateAfter)
{
